...
package com.hmxpanel.aiide.javac.services.partial

import com.hmxpanel.aiide.javac.services.compiler.JavacFlowListener
import com.hmxpanel.aiide.javac.services.util.ReparserUtils
import com.hmxpanel.aiide.javac.services.visitors.FindAnonymousVisitor
import com.hmxpanel.aiide.javac.services.visitors.TranslateMethodPositionsVisitor
...
class PartialReparserImpl : PartialReparser {
  ...
  override fun reparseMethod(
    ...
    try {
      val l = com.hmxpanel.aiide.javac.services.NBLog.instance(context)
  ...
  private fun newParser(
    ...
    val factory =
      com.hmxpanel.aiide.javac.services.NBParserFactory.instance(context)
        as com.hmxpanel.aiide.javac.services.NBParserFactory
    val scannerFactory = ScannerFactory.instance(context)
    val cancelService = com.hmxpanel.aiide.javac.services.CancelService.instance(context)
    val lexer = scannerFactory.newScanner(buf, true)
    if (
      endPositions
        is com.hmxpanel.aiide.javac.services.NBParserFactory.NBJavacParser.EndPosTableImpl
    ) {
      endPositions.resetErrorEndPos()
    }

    return object :
      com.hmxpanel.aiide.javac.services.NBParserFactory.NBJavacParser(
        factory,
        lexer,
        true,
        false,
